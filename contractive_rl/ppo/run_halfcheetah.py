"""
PPO on HalfCheetah-v4: StandardPPO vs ContractivePPO.

Run with:
  PYTHONPATH=/home/nvidia/.local/lib/python3.8/site-packages \
    /home/nvidia/anaconda3/envs/rlgpu/bin/python contractive_rl/ppo/run_halfcheetah.py
"""
...
def train_one_seed(variant, seed, cfg=CFG):
...
def run():
    ...
    print(f"Device: {DEVICE}  |  HalfCheetah-v4  |  {CFG['train_steps']//1000}k steps  |  hidden={HIDDEN_DIM}")
    print(f"{'='*60}")
    ...
    for variant in ("standard", "contractive"):
        ...
        for seed in SEEDS:
            ...
            ep_rets, val_log = train_one_seed(variant, seed)
    ...
    json_path = os.path.join(RESULTS_DIR, "halfcheetah_v4_ppo_results.json")
    ...
    axes[0].set_title(f"HalfCheetah-v4 — PPO ({len(SEEDS)} seeds)")
    ...
    fig.suptitle(f"ContractivePPO: V*(s)=b(s)/(1-eigs(s)), |eigs|<{SPECTRAL_BOUND}  |  hidden={HIDDEN_DIM}",
                 fontsize=10)
    fig.tight_layout()
    plot_path = os.path.join(RESULTS_DIR, "halfcheetah_v4_ppo_results.png")
    fig.savefig(plot_path, dpi=120); plt.close(fig)
    print(f"Saved: {plot_path}")

    print(f"\n{'='*60}")
    print("HALFCHEETAH-v4 PPO RESULTS")
    print(f"{'='*60}")
