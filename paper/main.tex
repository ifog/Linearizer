...
Two ablations test whether the contraction structure carries information.
\emph{Exp F} freezes $\eigs(s) \equiv \mathbf{0}$, collapsing $\zstar(s)=b(s)$
to a plain MLP (allowed by Prop.~\ref{prop:universal}); this isolates whether
the learned eigenvalues do useful work.
\emph{Exp G} replaces diagonal $\Lambda(s)$ with a dense $d\times d$ matrix
projected to spectral radius $<\spb$ via SVD clipping, with closed form by
linear solve $(\mathbf{I}-\Lambda(s))\zstar = b(s)$ at $\mathcal{O}(d^3)$ cost;
this isolates the value of off-diagonal coupling.
Results, alongside a plain-MLP baseline, are reported in
...
This is the empirical signature of Corollary~\ref{cor:bounded}: regardless of
optimizer pathology, $|Q(s,a)| \leq \|w_a\|_1 \cdot \|b(s)\|_\infty / (1-\spb)$
by construction, so divergence is structurally impossible as long as $b$ stays
bounded.
The spectral constraint thus provides a \emph{hard} stability guarantee absent
from standard architectures.
...
TD-learning instability under function approximation has been studied
extensively~\citep{baird1995residual,tsitsiklis1997analysis};
Gradient TD~\citep{sutton2009fast,maei2010toward} guarantees convergence for
linear approximators, while target networks~\citep{mnih2015human} and fitted
Q-iteration~\citep{ernst2005tree} address instability empirically.
Most directly, \citet{vincent2024projected} learn a \emph{projected} Bellman
operator end-to-end without spectral constraint or closed-form fixed point;
\citet{vincent2025iqn} jointly learn $K$ iterated Bellman updates, also
without structural contraction.
We instead enforce contraction by construction and read off $\zstar$ in
$\mathcal{O}(1)$.
...
weight matrices.
Deep equilibrium models~\citep{bai2019deq} define a layer's output as the
fixed point of a learned implicit operator;
\citet{winston2020monotone} enforce equilibrium uniqueness via monotone-operator
theory.
We combine both ideas: the Contractive Linearizer is a DEQ whose contraction
is simple enough ($\spb\tanh$ diagonal) to admit a \emph{closed-form} fixed
point, and the spectral constraint acts on the \emph{composed} operator
rather than on individual weight matrices.
We are unaware of prior work combining a DEQ-style equilibrium layer with a
