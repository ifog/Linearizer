...
on Pendulum-v0; results on MuJoCo HalfCheetah-v4 are reported in Section~\ref{sec:exp_ppo}.
...
\begin{enumerate}
  ...
  reduced by $55\%$ on CartPole and $41\%$ on Pendulum-v0, with $+13\%$ return
...
to prevent gradient spikes.

\paragraph{PPO (ContractiveCritic).}
The critic outputs the scalar value $V(s) = \zstar(s)$ directly.
...
All use the same MLP architecture for the feature extractor, with identical
hyperparameters.
We run 5 seeds on CartPole-v1 ($50$k steps) and Acrobot-v1 ($100$k steps).
...
We evaluate on CartPole-v1 (200k steps), Acrobot-v1 (500k steps),
Pendulum-v0 (300k steps, continuous), and HalfCheetah-v4
(1M steps, continuous, MuJoCo).
...
\begin{table}[t]
  ...
  \end{tabular}
  \vspace{0.5em}
  \small\textit{HalfCheetah-v4 results pending (1M steps, 3 seeds each).}
\end{table}
...
The contractive critic consistently reduces value-estimate variance across all
environments ($-55\%$ on CartPole, $-3\%$ on Acrobot, $-41\%$ on Pendulum).
...
\paragraph{Value function regularization.}
Several works regularize value functions toward stability.
\citet{kumar2020conservative} add a conservatism penalty for offline RL.
\citet{fujimoto2018addressing} clip the critic to reduce overestimation in actor-critic.
\citet{nikishin2022primacy} study catastrophic interference in online RL.
Our approach is orthogonal: we constrain the \emph{dynamics} of the Bellman
operator rather than regularizing the function's range.
...
architectural match rather than convergence properties.
The PPO improvement is modest on already-solved environments (CartPole) and
high-variance ones (Acrobot), suggesting the benefit is most pronounced in
challenging continuous control settings.

\paragraph{Future work.}
Extensions include full matrix (non-diagonal) contractive operators for richer
expressiveness, combining the contractive critic with distributional RL
~\citep{bellemare2017distributional}, and applying the closed-form inference to
real-time planning in robotics.
